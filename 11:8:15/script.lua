...
function GoodButtons( num,x,y,width,height,Tevent,Ccolors,toggle )
	print("Tevent")
	if (Tevent==nil) then
		Tevent=testEvent
	end
	...
	if (Gbutton[num]==nil) then
		Gbutton[num] = display.newRoundedRect(x,y,width,height,12)
		Gbutton[num].name=1
		...
		if (toggle==true) then
			Gbutton[num]:addEventListener("tap",toggleE)
